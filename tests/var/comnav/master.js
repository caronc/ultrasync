...
function buildMenu(active,ma0,ma1,ma2,ma3,ma4)
{
    ...
    if(ma0 == 1)
   {
     ...
     if(ma1 == 1)
    {
        ...
        if(active == 13)
            active = mcnt;
        mstring += '<li><a onclick="document.getElementById(\'newpage\').setAttribute(\'action\',\'/muser/textnames.htm\');document.getElementById(\'newpage\').submit(); ">Name Editor</a></li>';
    }
    mstring += '</ul>';
       d.innerHTML = mstring;
...
{
  return '["Arm/Disarm","Sensors","Output Control","History","Users","Email","SMS","Voice","Features","Network","Output Settings","IP Report","Name Editor"]';
}
...
function checkNameEntry(nid, length)
{
    ...
    var  notregex = new RegExp("[^abcdefghijklmnopqrstuvwxyz1234567890ABCDEFGHIJKLMNOPQRSTUVWXYZ !#$%&'()*+-.,/:;<=>?@[\\]^_{|}~\n\r]");     
 
    if(newData.match(notregex) != null)
    {
        alert("Data must only contain the following characters"+"abcdefghijklmnopqrstuvwxyz1234567890ABCDEFGHIJKLMNOPQRSTUVWXYZ !#$%&\'()*+-.,/:;<=>?@[\\]^_{|}~");
        newData = removeBadCharacters(newData,notregex);
